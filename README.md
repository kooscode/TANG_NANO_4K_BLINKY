...
The C++ project (mcu_project_blinky) flashes the onboard LED using the ARM Cortex M3 microcontroller and the Verilog project (fpga_project_blinky) will take over hardware and flash the LED faster using the FPGA when Button1 is pressed.
...
The BL702 is only used for USB to JTAG for programming and debugging the GW1NSR FPGA/MCU SOC.. __This is VERY dissapointing__ since this chip is actually a highly integrated BLE and zigbee combo chipset with a TON of Peripheral interfaces, which include USB2.0, IR-remote, SPI, UART, ISO 17987, I2C, I2S, PWM, QDEC, KeyScan, ADC, DAC, PIR, GPIOs and much more!!

The fact that this chip is ONLY used for the single purpose of JTAG programming makes me want to cry!! At No extra cost, this board could have had Bluetooth BLE, Zigbee and since it has 2x UARTS, it could even have provided Serial to USB for the Cortex M3!! That would have been awesome!!! Alas.. All pins are left unconnected, except for a lonesome few for JTAG and USB  (Insert sad crying emoji here.. )
...
- The `uart0_txd` and `uart0_rxd` are defined, but NOT USED in this project.. you can safely remove them if you want
...
- The main logic of this Verilog code module sits in the `always @(posedge clk_324) ` block. This is where a timer loop will toggle a LED buffer ON and OFF every 250ms and if Button1 is pushed, the physical LED will be mapped to the buffer and the LED will flash ON and OFF for as long as the button is held down.
- IMPORTANT : While Button1 is NOT pressed, the M3's GPIO[0] will be mapped to the physical LED, so the M3 has control over the LED while Button1 is NOT being held down.
...
- __You do NOT have to re-generate it__, but if you wan to see how its done, you can double click on the GPIO block to enable it and optionally the UART0 block and configure each.. This will generate the template code thats already inserted into the project..
...
- So - When the Verilog code is not asserting control over the LED, the M3's C++ code will have control and flash the LED.. But as soon as Button1 is pressed, the Verilog code will take over the LED mapping until Button1 is released.
...
Once the FPGA Bitstream and MCU Firmware has been uploaded, the code on the ARM Cortex M3 should automatically start to run and blink the LED about 2x per second.. Then when you press and hold Button1, the FPGA will take over the LED and flash it faster at about 4x per second!!
